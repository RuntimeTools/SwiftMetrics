          ...
          do {
            ...
            let tSMRCCtask2 = self.session.dataTask(with: urlRequest) { tSMRCCdata2, tSMRCCresponse2, tSMRCCerror2 in
              ...
              do {
                ...
                XCTAssertLessThan(tSMRCCresult2.time.data.start, currentTime, "Collection was created in the future")
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.time.data.start, minTime, "Collection created too long ago")
                XCTAssertLessThanOrEqual(tSMRCCresult2.time.data.end, currentTime, "Collection finished in the future")
                XCTAssertGreaterThan(tSMRCCresult2.time.data.end, minTime, "Collection finished too long ago")
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.time.data.end, tSMRCCresult2.time.data.start, "Collection started after it finished")
                expectCorrectTimeData.fulfill()
                // cpu values should be between 0 and 1
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.cpu.data.systemMean, 0, "CPU System Mean not big enough")
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.cpu.data.processMean, 0, "CPU Process Mean not big enough")
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.cpu.data.systemPeak, 0, "CPU System Peak not big enough")
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.cpu.data.processPeak, 0, "CPU Process Peak not big enough")
                XCTAssertLessThanOrEqual(tSMRCCresult2.cpu.data.systemMean, 1, "CPU System Mean not small enough")
                XCTAssertLessThanOrEqual(tSMRCCresult2.cpu.data.processMean, 1, "CPU Process Mean not small enough")
                XCTAssertLessThanOrEqual(tSMRCCresult2.cpu.data.systemPeak, 1, "CPU System Peak not small enough")
                XCTAssertLessThanOrEqual(tSMRCCresult2.cpu.data.processPeak, 1, "CPU Process Peak not small enough")
                // peaks should be bigger than means
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.cpu.data.systemPeak, tSMRCCresult2.cpu.data.systemMean, "CPU System Peak less than mean")
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.cpu.data.processPeak, tSMRCCresult2.cpu.data.processMean, "CPU Process Peak less than mean")
                // system should be higher than process
                // these tests cause false failures in Travis's linux container environment
#if !os(Linux)
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.cpu.data.systemPeak, tSMRCCresult2.cpu.data.processPeak, "CPU Process Peak higher than System Peak")
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.cpu.data.systemMean, tSMRCCresult2.cpu.data.processMean, "CPU Process Mean higher than System Mean")
#endif
                expectCorrectCPUData.fulfill()
                // mem values should be positive
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.memory.data.systemMean, 0, "Memory System Mean not big enough")
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.memory.data.processMean, 0, "Memory Process Mean not big enough")
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.memory.data.systemPeak, 0, "Memory System Peak not big enough")
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.memory.data.processPeak, 0, "Memory Process Peak not big enough")
                // peaks should be bigger than means
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.memory.data.systemPeak, tSMRCCresult2.memory.data.systemMean, "Memory System Peak less than mean")
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.memory.data.processPeak, tSMRCCresult2.memory.data.processMean, "Memory Process Peak less than mean")
                // system should be higher than process
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.memory.data.systemPeak, tSMRCCresult2.memory.data.processPeak, "Memory Process Peak higher than System Peak")
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.memory.data.systemMean, tSMRCCresult2.memory.data.processMean, "Memory Process Mean higher than System Mean")
                expectCorrectMemoryData.fulfill()
                // There should only be one HTTP record - for the creation POST call.
                XCTAssertEqual(1, tSMRCCresult2.httpUrls.data.count, "Only expected one HTTP record")
                XCTAssertEqual(1, tSMRCCresult2.httpUrls.data[0].hits, "Only expected one HTTP hit")
                XCTAssertEqual(self.collectionsEndpoint, tSMRCCresult2.httpUrls.data[0].url, "HTTP URL not \(self.collectionsEndpoint)/0")
                // HTTP times should be positive
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.httpUrls.data[0].averageResponseTime, 0, "HTTP Average response time not big enough")
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.httpUrls.data[0].longestResponseTime, 0, "HTTP Longest response time not big enough")
                // Longest should be higher than average
                XCTAssertGreaterThanOrEqual(tSMRCCresult2.httpUrls.data[0].longestResponseTime, tSMRCCresult2.httpUrls.data[0].averageResponseTime, "Memory Process Peak less than mean")
                print("\(tSMRCCresult2)")
                // cleanup
            ...
            let tSMRRCOPtask2 = self.session.dataTask(with: urlRequest2) { tSMRRCOPdata2, tSMRRCOPresponse2, tSMRRCOPerror2 in
              ...
              let tSMRRCOPtask3 = self.session.dataTask(with: urlRequest2) { tSMRRCOPdata3, tSMRRCOPresponse3, tSMRRCOPerror3 in
                ...
                do {
                  ...
                  XCTAssertLessThan(tSMRRCOPresult3.time.data.start, currentTime, "Collection was created in the future")
                  XCTAssertGreaterThanOrEqual(tSMRRCOPresult3.time.data.start, minTime, "Collection created too long ago")
                  XCTAssertLessThanOrEqual(tSMRRCOPresult3.time.data.end, currentTime, "Collection finished in the future")
                  XCTAssertGreaterThan(tSMRRCOPresult3.time.data.end, minTime, "Collection finished too long ago")
                  XCTAssertGreaterThanOrEqual(tSMRRCOPresult3.time.data.end, tSMRRCOPresult3.time.data.start, "Collection started after it finished")
                  // cpu values should be 0.0
                  XCTAssertEqual(tSMRRCOPresult3.cpu.data.systemMean, 0.0, "CPU System Mean too big")
                  XCTAssertEqual(tSMRRCOPresult3.cpu.data.processMean, 0.0, "CPU Process Mean too big")
                  XCTAssertEqual(tSMRRCOPresult3.cpu.data.systemPeak, 0.0, "CPU System Peak too big")
                  XCTAssertEqual(tSMRRCOPresult3.cpu.data.processPeak, 0.0, "CPU Process Peak too big")
                  // mem values should be 0
                  XCTAssertEqual(tSMRRCOPresult3.memory.data.systemMean, 0, "Memory System Mean too big")
                  XCTAssertEqual(tSMRRCOPresult3.memory.data.processMean, 0, "Memory Process Mean too big")
                  XCTAssertEqual(tSMRRCOPresult3.memory.data.systemPeak, 0, "Memory System Peak too big")
                  XCTAssertEqual(tSMRRCOPresult3.memory.data.processPeak, 0, "Memory Process Peak too big")
                  // There should only be one HTTP record - for the PUT call.
                  XCTAssertEqual(1, tSMRRCOPresult3.httpUrls.data.count, "Only expected one HTTP record")
                  XCTAssertEqual(1, tSMRRCOPresult3.httpUrls.data[0].hits, "Only expected one HTTP hit")
                  XCTAssertTrue(tSMRRCOPresult3.httpUrls.data[0].url.hasSuffix(tSMRRCOPuriString), "HTTP URL does not end with \(tSMRRCOPuriString)")
                  // HTTP times should be positive
                  XCTAssertGreaterThanOrEqual(tSMRRCOPresult3.httpUrls.data[0].averageResponseTime, 0, "HTTP Average response time not big enough")
                  XCTAssertGreaterThanOrEqual(tSMRRCOPresult3.httpUrls.data[0].longestResponseTime, 0, "HTTP Longest response time not big enough")
                  // Longest should be higher than average
                  XCTAssertGreaterThanOrEqual(tSMRRCOPresult3.httpUrls.data[0].longestResponseTime, tSMRRCOPresult3.httpUrls.data[0].averageResponseTime, "Memory Process Peak less than mean")
          ...
          let tSMRMHMcollectionID = String(tSMRMHMsplitUriString[tSMRMHMsplitUriString.count - 1])
            ...
            let tSMRMHMtask3 = self.session.dataTask(with: urlRequest2) { tSMRMHMdata3 , tSMRMHMresponse3, tSMRMHMerror3 in
              ...
              do {
                ...
                if let tSMRMHMreport3 = tSMRMHMresult3.httpUrls.data.first(where: { $0.url == self.collectionsEndpoint + "/" + tSMRMHMcollectionID }) {
