...
public class SwiftMetricsREST {
    ...
    private func httpEvent(http: HTTPData) {
        for key in smrCollectionList.keys {
            ...
            if let index = temp_httpUrlReports.data.index(where: { $0.url == http.url })  {
                ...
                temp_httpUrlReports.data.append(HttpUrlReport(url: http.url, hits: 1, averageResponseTime: http.duration, longestResponseTime: http.duration))
            }
